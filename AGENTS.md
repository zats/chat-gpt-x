...
The platform uses a small external launcher app. The launcher downloads the exact compatible runtime, binding, and extensions from version-addressed GitHub Releases. It does not bundle these components. When the user runs it, the launcher starts the stock, unmodified `ChatGPT.app` and injects the selected platform at process start. Once inside, the platform establishes a **stable, documented extension API** within the running app and **dynamically loads extensions** against that API.
...
- **Versioned bindings.** `src/platform/bindings/<app-version>/` bridges one specific ChatGPT build to one exact ChatGPT API version. One source directory holds the latest binding implementation for that build; binding versions are immutable release identities, not source-directory names. Its manifest declares its own `version`, `chatgpt`, `chatgptApi`, and the build's `app.asar` SHA-256. `src/platform/bindings/manifest.json` identifies the API-development binding and stock download URL used by CI. Later ChatGPT builds can remain in the release catalog on an older compatible API. For the same ChatGPT build, a compatible public-API addition increments the binding minor version, a breaking API change increments its major version, and a binding-only correction that keeps the same API increments its patch version. Never reuse or overwrite a published binding version. A new ChatGPT build gets a new source directory whose binding starts at `1.0.0`.
...
updates/
  latest.json                   # exact bindings and versioned API/extension catalog
.agents/skills/
...
7. Component releases are derived from predictable paths. Changes under the API, a versioned binding directory, or a public extension directory increment that component's semantic version and the schema-3 `updates/latest.json` generation in the same tested commit. Internal extensions declare `"private": true` and stay out of the public index. Bindings select exact ChatGPT builds by version and `app.asar` SHA-256. Extensions depend only on the ChatGPTX API range, so a new binding does not increment extension versions. The index retains every published public extension version. CI runs `scripts/refresh-update-index-hashes.sh <base-sha>` and includes deterministic archive hashes in the tested commit before validation. After CI passes on `main`, GitHub Releases publishes version-addressed `chatgpt-api-v<version>`, `binding-<chatgpt>-v<version>`, and `extension-<id>-v<version>` archives. CI downloads each archive and its checksum sidecar, verifies both against the catalog SHA-256, and then publishes the index on the stable `updates` release.
