 ...
 *
 * Usage: node src/platform/bindings/26.814.41957/ui-test.mjs [port]
...
async function validateUi(
  ...
  const openThreadMenu = async () => {
    ...
    try {
      ...
      const triggerState = (button) => ({
        connected: button.isConnected,
        threadId: button.getAttribute('data-cgptx-thread-id'),
        state: button.getAttribute('data-state'),
        expanded: button.getAttribute('aria-expanded'),
        height: button.getBoundingClientRect().height,
      });
      throw new Error(
        `Thread menu did not open: ${JSON.stringify({
          selected: triggerState(trigger),
          triggers: Array.from(
          ...
          menus: Array.from(document.querySelectorAll('[role="menu"]')).map(
            (candidate) => ({
              ...
              side: candidate.getAttribute('data-side'),
              threadIds: Array.from(candidate.children).flatMap((child) => {
                const threadId = child.getAttribute('data-cgptx-thread-id');
  ...
  await waitUntil(() => !threadMenu.isConnected);
