 ...
 *
 * Usage: node src/platform/bindings/26.831.21537/ui-test.mjs [port]
...
async function validateUi(
  ...
  const collapsedHeaderControls = Array.from(
    appHeader.querySelectorAll(
      ':scope > [data-test-id="header-shell-slot"]',
    ),
  ).at(-1);
  if (!collapsedHeaderControls) {
    throw new Error('Collapsed header control slot missing');
  }
  ...
  const threadHeaderRegion = appHeader?.querySelector(
    ':scope > [data-testid="app-shell-header-context-menu-surface"]',
  );
  if (!threadHeaderRegion) throw new Error('Thread header region missing');
  ...
  const expandedHeaderControls = Array.from(
    appHeader.querySelectorAll(
      ':scope > [data-test-id="header-shell-slot"]',
    ),
  ).at(-1);
  if (!expandedHeaderControls) {
    throw new Error('Expanded header control slot missing');
  }
  check(
    root.style.getPropertyValue('--header-background-color') ===
      initialHeaderColors.background &&
  ...
  const firstThreadSeparatorIndex = Array.from(threadMenu.children).findIndex(
    (child) =>
      ...
      Boolean(child.querySelector('.h-px[class*="bg-border"]')),
  );
  check(
