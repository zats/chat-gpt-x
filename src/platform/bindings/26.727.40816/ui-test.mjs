 ...
 *
 * Usage: node src/platform/bindings/26.727.40816/ui-test.mjs [port]
...
async function validateUi(
  ...
  let appHeader = document.querySelector(
  ...
  const headerToggle = (label) =>
    Array.from(
      document.querySelectorAll(
        `header[data-app-shell-header-edge-scroll] button[aria-label="${label}"]`,
      ),
  ...
  await waitUntil(() =>
    Boolean(document.querySelector('header[data-app-shell-header-edge-scroll]')),
  );
