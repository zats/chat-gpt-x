 ...
 *
 * Usage: node src/platform/bindings/26.730.61639/ui-test.mjs [port]
...
async function validateUi(
  ...
  const headerToggle = (label) =>
    Array.from(
      document.querySelectorAll(
        `header[data-app-shell-application-menu-bar] button[aria-label="${label}"]`,
      ),
  ...
  const collapsedHeaderControls = appHeader;
  ...
  const threadHeaderRegion = appHeader;
  ...
  const expandedHeaderControls = appHeader;
  check(
    root.style.getPropertyValue('--header-background-color') ===
      initialHeaderColors.background &&
      root.style.getPropertyValue('--header-foreground-color') ===
