# Derivation — bindings for 26.825.41651

Pinned build:

- App version: `26.825.41651`
- App build: `7345`
- app.asar SHA-256: `c089b63abb7ca4a751072c0da434248db13c32bed9c363e1b7e5428584b0576d`
- Electron: `151.0.7922.174`
- Sparkle enclosure: `https://persistent.oaistatic.com/codex-app-prod/ChatGPT-darwin-arm64-26.825.41651.zip`
- Binding date: `2026-08-29`
- Binding version: `1.0.0`
- ChatGPT API version: `1.5.2`
- Version-watcher issue: `#61`
...
`26.825.32147`, selected by `src/platform/bindings/manifest.json` at the start
of the rebind. It preserves that implementation's ChatGPT API `1.5.2` and
starts the target-build binding at version `1.0.0`.

## Verified module map

The shared implementations moved to `app-initial-Cw7fcqej.js`. Every asset
...
| --- | --- | --- |
| React, JSX, and React DOM | `app-initial-Cw7fcqej.js` | `U2t()` supplies React; `H2t()` supplies mutable `jsx` and `jsxs`; `z2t()` supplies `createRoot` |
| Native menus | `app-initial-Cw7fcqej.js` | initializer `aet`; namespace `net`; `net.Item`, `net.Separator`, `net.SubmenuItem`, and `net.FlyoutSubmenuItem`; `eet` dropdown root |
| Generic app menu | `app-initial-Cw7fcqej.js` | initializer `$9`; descriptor adapter `Q9`; current internationalization hook `c2t` |
| Assistant-selection toolbars | `app-initial-Cw7fcqej.js` | initializer `dy` and overlay `uy`; initializer `_w` and native positioner `gw` |
| Native icons | `app-initial-Cw7fcqej.js` | initializer `Wet` and chevron-right `Uet`; initializer `jv` and Profile icon `Av`; initializer `xot` and Settings icon `bot` |
| Native color picker | `app-initial-Cw7fcqej.js` | initializer `ds`; controlled picker `us` |
| Settings shell and search | `settings-page-D6FRxuR9.js` | native categories, sidebar rows, search results, pane selection, unsaved-navigation handling, and Suspense ownership |
| Settings section icons | `use-visible-settings-sections-Y8SX9KUp.js` | initializer `t`; section-icon map `r` |
| Settings breadcrumb | `toolbar-breadcrumb-jIYs1FZo.js` | initializer `n`; native breadcrumb component `t` |
| Native Settings page | `app-initial-Cw7fcqej.js` | initializer `Mo`; component `Ao` |
| Native Settings group, rows, and row | `app-initial-Cw7fcqej.js` | initializer `zn` and group `Rn`; initializer `QN` and rows `ZN`; initializer `oP` and row `rP` |
| Native Settings controls | `app-initial-Cw7fcqej.js` | initializer `det` and toggle `uet`; initializer `To`, section title `wo`, and select trigger `So`; initializer `stt` and button `ott`; initializer `hL` and controlled input `pL` |
| Native Settings loading row | `settings-loading-row-gP4hk9hW.js` | initializer `n`; component `t` |
| Application scope | `app-initial-Cw7fcqej.js` | initializer `Dzt`; application-scope token `Ezt`; scope hook `GUt` |
| App-server registry | `app-initial-Cw7fcqej.js` | initializer `pEt`; registry hook `gEt` |
| Query and message contracts | `app-initial-Cw7fcqej.js` | initializer `RLt` and query-client hook `zLt`; initializer `mLt` and query-key builder `dLt`; initializer `BWt` and message bus `VWt` |
| Browser and navigation bridges | `app-initial-Cw7fcqej.js` | open-in-browser dispatch `LWt`; navigation hook `vzt` |
| ChatGPT sign-in | `chatgpt-desktop-auth-url-BCznuctz.js` | initializer `r`; `o` creates the scoped ChatGPT login-session contract; `t` decorates the URL |
| Plus icon | `plus-BgCJgEEs-C-mgeBXA.js` | initializer `t`; component `n` |
| Palette icon | `palette-lzFbWMQk-Bagg5CnF.js` | initializer `n`; component `t` |
| Persisted-thread overflow | `thread-overflow-menu-CPZqPLUo.js` | initializer `n`; component `t`; shared generic adapter `Q9` remains in app-initial |
...
- `threadHeader.*`, `toggle-thread-pin`, and `copy-deeplink` identify current
  thread actions in `chatgpt-conversation-page-DL8SKZT7.js`,
  `remote-conversation-page-ClrI2NQ4.js`, and
  `thread-overflow-menu-CPZqPLUo.js`.
...
- `login-route-DyXCE7Sl.js` and
  `chatgpt-desktop-auth-url-BCznuctz.js` verify the current application-scope
...
action wrapper. Initializers `dy` and `_w` expose overlay `uy` and positioner
`gw`. The positioner supplies the selected-text rectangle, horizontal bounds,
portal target, live viewport position callback, and selection preservation on
pointer-down. Root actions normalize to `above` or `below`; child pages inherit
their parent's placement and replace only that placement's toolbar. Leaves
dismiss the selection and receive an immutable Command-key activation. Native
response-annotation creation, composer-preserving normal creation, and native
direct submission passed unchanged. The first-frame deferred geometry pass
keeps the below-selection surface visible and exactly `8` px below the selected
range after the native positioner's outer ref attaches.

The native UI validation re-resolves thread and Color flyout rows after each
forced appearance change, reopening the native Color submenu if ChatGPT closes
it during the appearance remount. The current connected rows preserve native
palette, focus, keyboard navigation, activation, and presentation contracts.
...
Colors loaded in normal order from the schema-3 lock. The target host reported
native-ready with no binding error and Settings navigation installed. Multiple
Accounts remained unavailable without a ChatGPT account identity, which is the
expected API-key-mode behavior.
...
node --check src/platform/bindings/26.825.41651/host.js
node --check src/platform/bindings/26.825.41651/ui-test.mjs
```

These reported version `26.825.41651`, build `7345`, Electron
...
`--test-api`, ran the binding-specific public layer, persisted and matched the
fresh renderer result, then launched the API suite together with shipped
extensions and ran the native layer. The exact build passed `44` extension and
utility checks, `34/34` applicable public API checks, and `45/45` native UI and
shipped-extension composition checks. The Release launcher built and signed
successfully, contained no bundled platform components, and staged the same
target binding source. API-key mode disabled only the harness-declared
