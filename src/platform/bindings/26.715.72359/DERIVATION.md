...
| Persisted sidebar row | `app-initial~app-main~onboarding-page~projects-index-page~hotkey-window-thread-page~chatgpt-~j34jmud9-DNuPQHcp.js` | `data-app-action-sidebar-thread-row`, scoped thread id/title attributes, and `data-thread-title-trigger` |
...
The local persisted-thread overflow and cloud thread menu components receive `conversationId`; the local component also supplies `title` and optional `cwd`. The binding wraps both through the same boundary. Cloud titles come from the matching native sidebar row. This supplies one thread-menu model and current-thread lifecycle for both thread kinds.
...
Native local and cloud sidebar rows retain their native trees and receive extension views at `data-thread-title-trigger`. A mutation observer covers rows rendered before and after injection. The absolute leading-view host grows leftward and leaves title geometry unchanged.

The same native UI suite selects a local fixture by id or a cloud fixture by `data-app-action-sidebar-thread-kind="remote"`. It exercises the same public API and assertions for both.
...
Header registrations still compose independently per property. The binding paints the native header regions, title, right-panel tab toolbar, and cloud header action surfaces. Cloud action backgrounds use a darker mix of the registered background while their text and borders derive from the registered foreground. Content-panel controls remain app-owned. The app's `electron-light` and `electron-dark` root classes select registered values.
...
- Version-specific native UI suite: `62/62` for both a seeded local thread and an authenticated cloud thread.
...
The Release build succeeded, its hardened-runtime ad-hoc signature verified, and packaged bridge/binding files matched source. The isolated burner-account flow passed the public suite `39/39`, the native suite `62/62`, account switching and restoration, and packaged-source verification. The current-account flow independently passed the same public and native suites against a cloud thread.
...
- Header/picker mismatch: header region topology, cloud action surface classes, theme root classes, React DOM root, or native picker export changed.
